...
<div class="flex items-center justify-between mb-8">
    <div>
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Edit Role: {{ ucfirst($role->name) }}</h1>
        </div>
        <p class="text-slate-500 dark:text-slate-400 mt-1">Toggle permissions for the <strong>{{ ucfirst($role->name) }}</strong> role. Changes affect all users with this role.</p>
    </div>
</div>
    ...
    <div class="space-y-6">
        ...
        <div class="glass-card rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex items-center justify-between">
                ...
                <button type="button" onclick="toggleGroup('{{ $group }}')" class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">Toggle All</button>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                @php
                    ...
                    $isAssigned = in_array($slug, $rolePermissions);
                @endphp
                <label class="flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-200 cursor-pointer hover:shadow-md {{ $isAssigned ? 'border-indigo-300 dark:border-indigo-500/40 bg-indigo-50/50 dark:bg-indigo-500/5' : 'border-slate-200 dark:border-slate-700 hover:border-indigo-200 dark:hover:border-indigo-500/20' }} {{ $isCritical ? 'opacity-50 cursor-not-allowed' : '' }}">
                    <input type="checkbox" 
                           name="permissions[]" 
                           value="{{ $slug }}" 
                           class="group-{{ $group }} w-5 h-5 rounded-lg border-slate-300 dark:border-slate-600 text-indigo-600 focus:ring-indigo-500 transition-all"
                           {{ $isAssigned ? 'checked' : '' }}
                           {{ $isCritical ? 'disabled' : '' }}>
                    <div class="flex-1">
...
function toggleGroup(group) {
    const checkboxes = document.querySelectorAll(`.group-${group}:not(:disabled)`);
